...
# define layout -------------------------------------------------------------------------------
img_zone = sg.Frame('', [[sg.Image(data=get_img(first=True), key='img_thum')]], size=(650,370))
...
convs = sg.Frame(' 変換拡張子 ',
         ...
         [sg.Button('MSP'), sg.Button('PCX')],
         [sg.Button('PPM'), sg.Button('SGI')],
         [sg.Button('TGA'), sg.Button('TIFF')],
         [sg.Button('JPEG'), sg.Button('ICNS')],
         [sg.Button('JPEG2000'), sg.Button('PNG')],
         [sg.Button('PDF'), sg.Button('WebP')]
         ]
         , vertical_alignment='t',size=(300,370))

output_frame = sg.Frame('',[[sg.ProgressBar(100,orientation='h', size=(97,15), pad=((5,5),(10,0)))],
        [sg.Output(size=(150,6), pad=((5,5),(10,5)), text_color='white',background_color='black')]], pad=((5,5),(5,10)),size=(810,140))

layout = [  
            [sg.Text('  1. 元画像を選択     2. 保存先を選択     3. 保存ファイル名を入力     4. 拡張子を選択     5. 変換開始ボタン押下  ')],
            [sg.Text('  元画像  '), sg.InputText(key='file1', enable_events=True, default_text='画像のパスを入力'), sg.FileBrowse(' 参照 ')],
            [sg.Text('  保存先  '), sg.InputText(key='folder1', enable_events=True, default_text='保存先フォルダのパスを入力'), sg.FolderBrowse(' 参照 ')],
            [sg.Text('  保存名  '), sg.InputText(key='save_name', default_text='保存するファイル名を入力')],
            [img_zone, convs],
            [sg.Button('  変換開始  ')],
            [output_frame]
        ]
...
while True:
    ...
    if event == sg.WIN_CLOSED:
        ...
        extension = '.icns'
        print('変換対象拡張子：ICNSを選択')
    elif event == 'JPEG2000': #no
        extension = '.jp2'
